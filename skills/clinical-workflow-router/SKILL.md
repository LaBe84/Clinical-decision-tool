...
keep someone talking, how to obtain a location), and it should not
characterise the required response level either (e.g. "already requiring the
highest level of response") — that's a triage/response-level judgement
belonging to the people and procedures actually handling the situation, not
a routing statement. "The immediate-safety override applies; organisational
procedure takes precedence" is the complete statement — nothing more is
this skill's to add. That's exactly the kind of role a routing skill
shouldn't drift into, and it's covered by
...
**The high-contact pattern (OCD pattern):** a case
...
1. **Contact Pattern Analysis always comes first** for this pattern —
   ...
   function. Nothing downstream can be decided honestly without this. When
   describing the starting position, be precise: "no formulation provided in
   the material" or "whether a formulation already exists is not
   established" are accurate; "no formulation" stated as a fact is not,
   unless the material actually establishes that — don't let "not present in
   what I've been given" slide into "doesn't exist," which is the exact
   unknown-as-absence error named two paragraphs below.
...
risk judgement or a response recommendation. But this pattern isn't a fixed
two-step sequence either — it depends on whether direct enquiry has already
happened:

- Where suicide-related material is present and direct enquiry has **not**
  already been completed, **Suicide Enquiry** establishes what's known,
  organised by CASE chronology, before **PORF Formulation** interprets it.
- Where completed, documented direct enquiry is already established in the
  material, route straight to **PORF Formulation** — don't insert Suicide
  Enquiry as a mandatory first step just because the case involves
  suicidality, per the anti-recursion rule above. Suicide Enquiry only
  re-enters if there's concrete evidence of a gap, or completeness itself is
  the question being asked.

When Suicide Enquiry does run in this pattern, its input is the specific
suicide-information gap identified from the material (or from whatever
analysis preceded it) — not PORF's output. The dependency runs one direction
only: Suicide Enquiry's CASE-organised output feeds PORF, never the reverse.
...
- **"Suicide was asked about" does not automatically establish that CASE
  ...
  This holds regardless of what form the material arrives in — a completion
  summary, raw counsellor notes, or a transcript — as long as it states or
  clearly shows that direct enquiry covering thoughts, plan, and history
  occurred. Don't invent a distinction between "the enquiry was done" and
  "but it hasn't been run through Suicide Enquiry's structured CASE format
  yet" as a reason to route there anyway — reformatting completed material
  into CASE structure is not itself a reason to re-run enquiry, and treating
  it as one recreates the exact recursive-audit failure this rule exists to
  close. Route straight to PORF; if organising the material into CASE form
  would help, that's a formatting step, not a routing requirement.
...
- When routing to none of the five, keep the reasoning at the routing level:
  "no [X]-routing question is established in the material provided" — not a
  clinical judgement about whether the underlying situation warrants
  enquiry, formulation, or intervention. "Nothing here presents a suicide-
  enquiry routing question" is this skill's territory; "this level of low
  mood doesn't warrant direct suicide enquiry" is a clinical/policy judgement
  this skill has no business making. Don't let "not routed to" quietly
  become "clinically unnecessary."
